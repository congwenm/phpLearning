...
try{
	/*Added for checking APP ID and APP SECRET so 
	that only our own front-end clients are able to make request*/
	...
	//decrypt the request
	$params = json_decode(trim(mcrypt_decrypt(
		MCRYPT_RIJNDAEL_256, 
		$applications[$app_id],
	...
	//get the controller and format it correctly so the first letter 
	// is always capitalized
	$controller = ucfirst(strtolower($params['controller']));

	//get the action and format it correctly so all the letters are not capitalized
	// and append 'Action'
